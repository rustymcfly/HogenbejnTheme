...
class OfferItemCartProcessor implements CartProcessorInterface, CartDataCollectorInterface
{
    private ProductCartProcessor $decorated;
        ...
        Profiler::trace('cart::product::collect', function () use ($data, $original, $context, $behavior): void {
            ...
            if (!empty($ids)) {
                ...
                foreach ($products as $product) {
                    $product->setChildren(new SalesChannelProductCollection());
    ...
    public function process(CartDataCollection $data, Cart $original, Cart $toCalculate, SalesChannelContext $context, CartBehavior $behavior): void
    {
        Profiler::trace('cart::product::process', function () use ($data, $original, $toCalculate, $context): void {
            ...
            $offerItems = $original->getLineItems()->filterFlatByType(OfferLineItemFactory::TYPE);
            if (count($offerItems)) {
                $offerItem = $offerItems[0];
                $taxCollection = new CalculatedTaxCollection();
                $taxRuleCollection = new TaxRuleCollection();
                $offerItem->setPrice(new CalculatedPrice(0, 0, $taxCollection, $taxRuleCollection));
                $toCalculate->add($offerItems[0]);
            }
    ...
    private function validateTimestamp(LineItem $item, Cart $cart, CartDataCollection $data, CartBehavior $behavior, LineItemCollection $items): void
    {
        ...
        if ($behavior->hasPermission(ProductCartProcessor::KEEP_INACTIVE_PRODUCT)) {
            return;
        }
    ...
    {
        if ($behavior->hasPermission(ProductCartProcessor::SKIP_PRODUCT_STOCK_VALIDATION)) {
            return;
        }
    ...
    private function shouldPriceBeRecalculated(LineItem $lineItem, CartBehavior $behavior): bool
    {
        if ($lineItem->getPriceDefinition() !== null
            && $lineItem->hasExtension(ProductCartProcessor::CUSTOM_PRICE)
            && $behavior->hasPermission(ProductCartProcessor::ALLOW_PRODUCT_PRICE_OVERWRITES)) {
            return false;
        }

        if ($lineItem->getPriceDefinition() !== null
            && $behavior->hasPermission(ProductCartProcessor::SKIP_PRODUCT_RECALCULATION)) {
            return false;
        }
    ...
    public function setDecorated(ProductCartProcessor $cartProcessor)
    {
        $this->decorated = $cartProcessor;
    }

    public function getDecorated(): ProductCartProcessor
    {
